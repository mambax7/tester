 ...
 * @version         $Id: 2.10 request.php 11532 Mon 2014/04/28 11:10:05Z XOOPS Development Team $
 */
 ...
 */
define('XOALBUM_REQUEST_NOTRIM', 1);
define('XOALBUM_REQUEST_ALLOWRAW', 2);
define('XOALBUM_REQUEST_ALLOWHTML', 4);

/**
 * XoalbumRequest Class
 * This class serves to provide a common interface to access
 * request variables.  This includes $_POST, $_GET, and naturally $_REQUEST.  Variables
 * can be passed through an input filter to avoid injection or returned raw.
 */
class XoalbumRequest
{
    ...
    public static function getVar($name, $default = null, $hash = 'default', $type = 'none', $mask = 0)
    {
        ...
        if (isset($input[$name]) && $input[$name] !== null) {
            ...
            $var = XoalbumRequest::_cleanVar($input[$name], $mask, $type);
            // Handle magic quotes compatability
            if (get_magic_quotes_gpc() && ($var != $default) && ($hash != 'FILES')) {
                $var = XoalbumRequest::_stripSlashesRecursive($var);
            }
        } elseif ($default !== null) {
            // Clean the default value
            $var = XoalbumRequest::_cleanVar($default, $mask, $type);
        } else {
            $var = $default;
        }
    ...
    {
        return XoalbumRequest::getVar($name, $default, $hash, 'int');
    }
    ...
    {
        return XoalbumRequest::getVar($name, $default, $hash, 'float');
    }
    ...
    {
        return XoalbumRequest::getVar($name, $default, $hash, 'bool');
    }
    ...
    {
        return XoalbumRequest::getVar($name, $default, $hash, 'word');
    }
    ...
    {
        return XoalbumRequest::getVar($name, $default, $hash, 'cmd');
    }
    ...
    public static function getString($name, $default = '', $hash = 'default', $mask = 0)
    {
        ...
        return (string) XoalbumRequest::getVar($name, $default, $hash, 'string', $mask);
    }
    /**
     * @param        $name
     * @param array  $default
    ...
    {
        return XoalbumRequest::getVar($name, $default, $hash, 'array');
    }
    ...
    {
        return (string) XoalbumRequest::getVar($name, $default, $hash, 'string', PUBLISHER_REQUEST_ALLOWRAW);
    }
    ...
    public static function get($hash = 'default', $mask = 0)
    {
        ...
        $result = XoalbumRequest::_cleanVar($input, $mask);
        // Handle magic quotes compatability
        if (get_magic_quotes_gpc() && ($hash != 'FILES')) {
            $result = XoalbumRequest::_stripSlashesRecursive($result);
        }
    ...
    {
        foreach ($array as $key => $value) { XoalbumRequest::setVar($key, $value, $hash, $overwrite);
        }
    }
    ...
    {
        XoalbumRequest::_cleanArray($_FILES);
        XoalbumRequest::_cleanArray($_ENV);
        XoalbumRequest::_cleanArray($_GET);
        XoalbumRequest::_cleanArray($_POST);
        XoalbumRequest::_cleanArray($_COOKIE);
        XoalbumRequest::_cleanArray($_SERVER);
        if (isset($_SESSION)) { XoalbumRequest::_cleanArray($_SESSION);
        }
        $REQUEST = $_REQUEST;
        $GET = $_GET;
        $POST = $_POST;
    ...
    public static function _cleanVar($var, $mask = 0, $type = null)
    {
        ...
        if ($mask & 2) {
            ...
            if (is_null($safeHtmlFilter)) {
                $safeHtmlFilter = XoalbumFilterInput::getInstance(null, null, 1, 1);
            }
            $var = $safeHtmlFilter->clean($var, $type);
        } else {
            // Since no allow flags were set, we will apply the most strict filter to the variable
            if (is_null($noHtmlFilter)) {
                $noHtmlFilter = XoalbumFilterInput::getInstance( /* $tags, $attr, $tag_method, $attr_method, $xss_auto */);
            }
    ...
    {
        $value = is_array($value) ? array_map(array('XoalbumRequest', '_stripSlashesRecursive'), $value) : stripslashes($value);
...
/**
 * XoalbumFilterInput is a class for filtering input from any data source
 * Forked from the php input filter library by: Daniel Morris <dan@rootcube.com>
 * Original Contributors: Gianpaolo Racca, Ghislain Picard, Marco Wandschneider, Chris Tobin and Andrew Eddie.
 *
 * @author      Louis Landry <louis.landry@joomla.org>
 */
class XoalbumFilterInput
{
     ...
     *      <pre>  $filter = & XoalbumFilterInput::getInstance();</pre>
     *
     * @static
     *
     * @param   array   $tagsArray  list of user-defined tags
     ...
     * @param   int     $xssAuto    Only auto clean essentials = 0, Allow clean blacklisted tags/attr = 1
     *
     * @return object The XoalbumFilterInput object.
     * @since   1.5
     */
    public static function & getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
    {
        ...
        if (empty ($instances[$sig])) {
            $instances[$sig] = new XoalbumFilterInput($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
        }
    ...
    public function clean($source, $type = 'string')
    {
        ...
        switch (strtoupper($type)) {
            ...
            case 'STRING' :
                ...
                if (isset($this) && is_a($this, 'XoalbumFilterInput')) {
                    $filter =& $this;
                } else {
                    $filter = XoalbumFilterInput::getInstance();
                }
            ...
            case 'PATH' :
                ...
                break;
            case 'USERNAME' :
                $result = (string) preg_replace('/[ -<>"\'%&]/', '', $source);
                break;
            default :
                // Check for static usage and assign $filter the proper variable
                if (is_object($this) && get_class($this) == 'XoalbumFilterInput') {
                    $filter =& $this;
                } else {
                    $filter = XoalbumFilterInput::getInstance();
                }
    ...
    protected function cleanAttributes($attrSet)
    {
        ...
        for ($i = 0; $i < count($attrSet); ++$i) {
            ...
            if (XoalbumFilterInput::checkAttribute($attrSubSet)) {
                continue;
            }
